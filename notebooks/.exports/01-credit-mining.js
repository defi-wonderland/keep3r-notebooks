...
next(async () => {
  ...
  console.log(
    'KP3R minted by inflation period',
    bnToNumber((await keep3r.v2.jobPeriodCredits(job.address)).mul(inflationPeriod).div(rewardPeriodTime))
  );
});
...
next(async () => {
  await $.sleepAndRecord(3 * rewardPeriodTime, $.time(4, 'hours'));
...
next(async () => {
  ...
  await $.sleepAndExecute(4 * rewardPeriodTime, $.time(4, 'hours'), [
    {
      run: async () => {
...
next(async () => {
  ...
  await $.sleepAndExecute(2 * rewardPeriodTime, $.time(4, 'hours'), [
    {
      run: async () => {
